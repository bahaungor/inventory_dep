...
router.post(
  ...
  create_post, // ACCESS UPLOADED FILE IN CONTROLLER FUNC VIA req.file
);
router.get('/inventory/:selected/:name', item_get);
router.delete('/:selected/:id/', item_delete);
router.patch(
  '/:selected/:id', // WHEN '/${selected}/${id}' GETS PATCH REQUEST
  upload.single('image'), // PARSE 'image' FIELD IN REQUEST BODY
  createValidator, // VALIDATE FIELDS IN req.body AFTER MULTER !!!
  item_modify, // ACCESS UPLOADED FILE IN CONTROLLER FUNC VIA req.file
);
